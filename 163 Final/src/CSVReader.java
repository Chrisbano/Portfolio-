...
// STUDENTS
// this code is provided incase you don't have a working CSVReader by this point. 
// You should use the one you implemented for P3 and P4, but here to help. 
...
public class CSVReader {
    ...
    public List<String> getNext() {
      /*** more common version
       ...
       return null;
       */
       /** this code below is how to allow quoted lines, and the 'proper' way to implement it.  **/
       if(hasNext()){
           String toSplit = fileScanner.nextLine();
           ...
           for (int current = 0; current < toSplit.length(); current++) {
               ...
               if (atLastChar) {
                   result.add(toSplit.substring(first).replace("\"", "")); // remove the quotes from the quoted item
               } else {
                   if (toSplit.charAt(current) == DELIMINATOR && !inQuotes) {
                       result.add(toSplit.substring(first, current).replace("\"", ""));
                       first = current + 1;
                   }
               }
           }
